...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            #TODO: Store data from the environment in the buffers
            #BEGIN SOLN
            obs[step] = next_obs
        ...
        b_mc_returns = mc_returns.reshape(-1)
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.rl_alg == "reinforce":
                    with torch.no_grad():
                        ...
                        clipfracs += [torch.tensor(0)]
                     
                    pg_loss = b_mc_returns[mb_inds] * newlogprob
                    pg_loss = -1.0 * pg_loss.mean()
                    #ignore the below
                    v_loss = torch.tensor(0.0)
                    v_loss_clipped = torch.tensor(0.0)
                    entropy_loss = entropy.mean()
                    explained_var = torch.tensor(0.0)
                    loss = -1.0 * pg_loss
                elif args.rl_alg == "ppo":
